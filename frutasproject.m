...
try
    if carga | recortada  
        ...
        gmat=Im(:,:,2);
        bmat=Im(:,:,3);
        %Poner niveles a cada RGB para la funcion bina  rize
        levelr=0.95;
        levelg=0.98;
        levelb=0.5;
        ...
        %Activar botones de contornos
        set(handles.edges_detectionS,'Enable','on')
        set(handles.edges_detectionC,'Enable','on')
        imagenNueva=Isum;
    else
        msgbox('No se ha detectado ninguna imagen','Error','error')
        set(handles.fill_holes,'Enable','off')
        set(handles.edges_detection,'Enable','off')
        set(handles.edges_detectionC,'Enable','off')
    end
catch
    msgbox('Primero limpie la imagen, haciendo clic en el botón limpiar','Error','error')
    set(handles.fill_holes,'Enable','off')
    set(handles.edges_detectionS,'Enable','off')
    set(handles.edges_detectionC,'Enable','off')
end
...
global imagen 
global imagenNueva
%Funcion strel es morfologica para reconocer el objeto
se = strel('disk',70);
...
% handles    structure with handles and user data (see GUIDATA)
global imagenNueva
%Ayuda a detectar el objeto en la imagen
...
imshow(imagenNueva);
%Decirle a Matlab que espere y haga lo siguiente antes de mostrar la imagen
hold on;
...
%Continua y hace lo siguiente y muestra la imagen
hold off;
set(handles.better_holes,'Enable','off');
...
% --- Executes on button press in edges_detectionS.
%Detectar los contornos de la imagen con Sobel, en este caso las frutas
function edges_detectionS_Callback(hObject, eventdata, handles)
% hObject    handle to edges_detectionS (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global imagenNueva
I=imagenNueva;
%Filtro sobel, necesita escala grises
filtrosobel= edge(I, 'sobel');
imshow(filtrosobel), title("Filtro Sobel de contornos");
%Desactivar boton de contorno
set(handles.edges_detectionS,'Enable','off')

% --- Executes on button press in edges_detectionC.
%Detectar los contornos de la imagen con Canny, en este caso las frutas
function edges_detectionC_Callback(hObject, eventdata, handles)
% hObject    handle to edges_detectionC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global imagenNueva
I=imagenNueva;
%Filtro canny, necesita escala grises
filtrocanny= edge(I, 'canny');
imshow(filtrocanny), title("Filtro Canny de contornos");
%Desactivar boton de contorno
set(handles.edges_detectionC,'Enable','off')
